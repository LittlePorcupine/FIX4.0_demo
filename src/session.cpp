...
void Session::send_buffered_data() {
    ...
    // Check if we can finally close
    if (shutting_down_ && outbound_q_.empty()) {
...
void Session::on_shutdown(const std::string& reason) {
    perform_shutdown("Shutdown requested: " + reason);
}
...
void Session::perform_shutdown(const std::string& reason) {
    ...
    stop(); // Immediately signal that the session is no longer running.

    std::cout << "[Session] Shutting down: " << reason << std::endl;
    
    // Create logout message. The sequence number is applied in send()
    auto logout_msg = create_logout_message(senderCompID, targetCompID, 0, reason);
    send(logout_msg);
    
    // After sending logout, if the outbound queue is empty, we can trigger shutdown immediately.
    // Otherwise, it will be triggered by send_buffered_data when the queue is clear.
